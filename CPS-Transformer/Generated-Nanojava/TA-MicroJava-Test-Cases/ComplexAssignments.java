...
new CATest ( ) . ____Main____ ( new Continuation()  ) ; } }  class CATest { int
   ...
   a ;  int  x ;  CATest  ___pseudoThis ;
   ___ContinuationClasscontinuationMethod0  ___k1 ; ___pseudoThis = this;  a =
   10;  b = 20;  arr = new int[((a * b) + (b - a))];  System.out.println ( arr.
   length ) ;  x = arr.length - 1;  System.out.println ( arr [ x ] ) ;  arr[2]
   = (((a + b) * (b)));  arr[0] = ((arr [ 2 ]) + ((arr [ 2 ]) + (a * b))); if (
   ((arr [ 0 ]) < (arr [ 2 ])) ) { arr[3] = ((arr [ 0 ]) - 3);  ___k1 = new
   ...
   ___k ) { CATest  ___pseudoThis ; ___pseudoThis = this;  System.out.println (
   arr [ 3 ] ) ; ___k.call(); }}
