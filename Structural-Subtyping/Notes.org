...
***** When the parity is one, check whether B <= A instead of A <= B
...
****** TODO During DFS for A <= B, if parity is 0, getNeighbours will return those neighbours connected by edges of B. Else, those connected by edges of A.
